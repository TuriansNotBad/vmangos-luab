...
bool PlayerBotMgr::LuaDofile(const std::string& filename) {
    if (luaL_dofile(L, filename.c_str()) != LUA_OK) {
        m_luaCease = true;
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Lua error executing file %s: %s\n", filename.c_str(), lua_tostring(L, -1));
...
void PlayerBotMgr::LuaReloadFiles() {
    if (!L) return;
    std::string path = "aiscripts";

    // load prereqs
    if (!LuaDofile(path + "/logic_list.lua"))
        return;
    if (!LuaDofile(path + "/goal_list.lua"))
        return;
    if (!LuaDofile(path + "/ai_define.lua"))
        return;
...
void PlayerBotMgr::LuaReloadInternal() {
    m_luaReload = false;
    if (L) {
        lua_close(L);
        L = nullptr;
...
void PlayerBotMgr::Update(uint32 diff)
{
    ...
    if (m_luaReload) {
        ...
        for (auto iter = m_bots.begin(); iter != m_bots.end();) {
            if (iter->second->state == PB_STATE_ONLINE
                && iter->second->ai
                && iter->second->ai->me
                && iter->second->ai->IsLuaBot())
            {
                LuaBotAI* ai = dynamic_cast<LuaBotAI*>(iter->second->ai.get());
                if (!ai) continue;
                ai->Reset(true);
                ai->CeaseUpdates(false);
            }
            ++iter;
        }
        LuaReloadInternal();
        m_luaReload = false;
        sWorld.SendServerMessage(ServerMessageType::SERVER_MSG_CUSTOM, "Lua reload complete.");
    ...
    // LUA BOT START
    if (!m_luaCease) {
        if ((m_elapsedTime - m_luaUpdateLast) > m_luaUpdateInterval) {
            for (auto iter = m_bots.begin(); iter != m_bots.end();) {
                if (iter->second->state == PB_STATE_ONLINE
                    && iter->second->ai
                    && iter->second->ai->me
                    && iter->second->ai->IsLuaBot())
                {
                    (dynamic_cast<LuaBotAI*>(iter->second->ai.get()))->UpdateAILua(diff);
                }
                iter++;
            }
            m_luaUpdateLast = m_elapsedTime;
        }
    }
    // LUA BOT END
...
void PlayerBotMgr::ReviveAll(Player* pMaster)
{
    if (!pMaster) return;

    Group* group = pMaster->GetGroup();
    if (!group) return;

    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
        if (Player* pMember = itr->getSource())
            if (pMember->AI() && pMember->AI()->IsLuaBot() && !pMember->IsAlive())
                pMember->ResurrectPlayer(1.0f, false);
...
bool ChatHandler::HandleBotDeleteAllCommand(char * args)
{
...
bool HandlePartyBotAddCommand_HelperClassNameToClassID(char* in, uint8& botClass, Team playerTeam) {
    int tempBotClass = -1;
    std::string option = in;
    if (option == "warrior")
        tempBotClass = CLASS_WARRIOR;
    else if (option == "paladin" && playerTeam == ALLIANCE)
        tempBotClass = CLASS_PALADIN;
    else if (option == "hunter")
        tempBotClass = CLASS_HUNTER;
    else if (option == "rogue")
        tempBotClass = CLASS_ROGUE;
    else if (option == "priest")
        tempBotClass = CLASS_PRIEST;
    else if (option == "shaman" && playerTeam == HORDE)
        tempBotClass = CLASS_SHAMAN;
    else if (option == "mage")
        tempBotClass = CLASS_MAGE;
    else if (option == "warlock")
        tempBotClass = CLASS_WARLOCK;
    else if (option == "druid")
        tempBotClass = CLASS_DRUID;
    if (tempBotClass == -1)
        return false;
    botClass = (uint8) tempBotClass;
    return true;
}

bool HandlePartyBotAddCommand_HelperRoleNameToVals(char* in, uint8& botClass, CombatBotRoles& botRole, Team playerTeam) {
    std::string option(in);
    if (option == "dps")
    {
...
bool HandlePartyBotAddCommand_HelperRaceNameToRaceID(char* in, uint8 botClass, uint8& botRace, Team playerTeam) {
    int tempBotRace = -100;
    std::string option = in;
    if (playerTeam == ALLIANCE) {
        if (botClass == CLASS_SHAMAN)
            return false;
        if (option == "human" && !(botClass == CLASS_DRUID || botClass == CLASS_HUNTER)) {
            botRace = RACE_HUMAN;
            return true;
        }
        else if (option == "dwarf" && !(botClass == CLASS_DRUID || botClass == CLASS_MAGE || botClass == CLASS_WARLOCK)) {
            botRace = RACE_DWARF;
            return true;
        }
        else if (option == "gnome" && !(botClass == CLASS_DRUID || botClass == CLASS_PALADIN || botClass == CLASS_PRIEST || botClass == CLASS_HUNTER)) {
            botRace = RACE_GNOME;
            return true;
        }
        else if (option == "elf" && !(botClass == CLASS_PALADIN || botClass == CLASS_MAGE || botClass == CLASS_WARLOCK)) {
        ...
        if (botClass == CLASS_PALADIN)
            return false;
        if (option == "orc" && !(botClass == CLASS_DRUID || botClass == CLASS_MAGE || botClass == CLASS_PRIEST)) {
            botRace = RACE_ORC;
            return true;
        }
        else if (option == "tauren" && !(botClass == CLASS_PRIEST || botClass == CLASS_MAGE || botClass == CLASS_WARLOCK || botClass == CLASS_ROGUE)) {
            botRace = RACE_TAUREN;
            return true;
        }
        if (option == "troll" && !(botClass == CLASS_DRUID || botClass == CLASS_WARLOCK)) {
            botRace = RACE_TROLL;
            return true;
        }
        else if (option == "undead" && !(botClass == CLASS_DRUID || botClass == CLASS_HUNTER || botClass == CLASS_SHAMAN)) {
            botRace = RACE_UNDEAD;
            return true;
        }
    }
...
bool HandlePartyBotAddCommand_HelperRoleNameToRoleID(char* in, Classes botClass, CombatBotRoles& botRole, Team playerTeam) {
    std::string option(in);
    if (option == "dps") {
        botRole = CombatBotBaseAI::IsMeleeDamageClass(botClass) ? ROLE_MELEE_DPS : ROLE_RANGE_DPS;
        return true;
        ...
        if (botClass != CLASS_PRIEST && botClass != CLASS_DRUID)
            if (playerTeam == HORDE && botClass != CLASS_SHAMAN)
                return false;
            else if (playerTeam == ALLIANCE && botClass != CLASS_PALADIN)
                return false;
        botRole = ROLE_HEALER;
        return true;
    }
    else if (option == "tank") {
        if (botClass != CLASS_DRUID && botClass != CLASS_PALADIN && botClass != CLASS_WARRIOR)
            return false;
        botRole = ROLE_TANK;
        return true;
    }
...
bool ChatHandler::HandlePartyBotAddCommand(char* args)
{
    ...
    int lua_logic_id = -1;
    int lua_gender = -1;
    uint8 luabotrace = RACE_HUMAN;

    if (char* arg1 = ExtractArg(&args))
    {
        std::string option = arg1;
        // check if arg is class name
        if (!HandlePartyBotAddCommand_HelperClassNameToClassID(arg1, botClass, pPlayer->GetTeam()))
            // check if arg is role name
            HandlePartyBotAddCommand_HelperRoleNameToVals(arg1, botClass, botRole, pPlayer->GetTeam());

        if (option == "lua")
        {
            isLuaBot = true;
            if (char* luaClassOption = ExtractArg(&args)) {
                if (!HandlePartyBotAddCommand_HelperClassNameToClassID(luaClassOption, botClass, pPlayer->GetTeam())) {
                    SendSysMessage("Incorrect class. Allowed values - warrior, paladin, hunter, rogue, priest, shaman, mage, warlock, druid.");
                    SetSentErrorMessage(true);
                    return false;
                }
            }
            else {
                SendSysMessage("Incorrect syntax. Expected class.");
                SetSentErrorMessage(true);
                return false;
            }

            if (char* luaRoleOption = ExtractArg(&args)) {
                if (!HandlePartyBotAddCommand_HelperRoleNameToRoleID(luaRoleOption, (Classes) botClass, botRole, pPlayer->GetTeam())) {
                    SendSysMessage("Incorrect role. Allowed values - dps, healer, tank.");
                    SetSentErrorMessage(true);
                    return false;
                }
            }
            else {
                SendSysMessage("Incorrect syntax. Expected role.");
                SetSentErrorMessage(true);
                return false;
            }

            if (char* luaRaceOption = ExtractArg(&args)) {
                if (!HandlePartyBotAddCommand_HelperRaceNameToRaceID(luaRaceOption, botClass, luabotrace, pPlayer->GetTeam())) {
                    SendSysMessage("Incorrect race name.");
                    SetSentErrorMessage(true);
                    return false;
                }
            }
            else {
                SendSysMessage("Incorrect syntax. Expected race name.");
                SetSentErrorMessage(true);
                return false;
            }

            if (!ExtractInt32(&args, lua_logic_id)) {
                SendSysMessage("Incorrect syntax. Expected logic id.");
                SetSentErrorMessage(true);
                return false;
            }
        }
...
bool ChatHandler::HandlePartyBotAddLuaBCommand(char * args)
{
    Player* pPlayer = m_session->GetPlayer();
    if (!pPlayer)
        return false;
    ...
    if (char* luaRaceOption = ExtractArg(&args)) {
        if (!HandlePartyBotAddCommand_HelperRaceNameToRaceID(luaRaceOption, botClass, botRace, pPlayer->GetTeam())) {
            SendSysMessage("Incorrect race name.");
            SetSentErrorMessage(true);
            return false;
        }
    ...
    if (!ExtractInt32(&args, botGender)) {
        SendSysMessage("Incorrect syntax. Expected gender id.");
        SetSentErrorMessage(true);
        return false;
    }

    if (!ExtractInt32(&args, botLogicID)) {
        SendSysMessage("Incorrect syntax. Expected logic id.");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    std::string botSpec = "";
    if (char* cstrBotSpec = ExtractArg(&args))
        botSpec = cstrBotSpec;
    ...
    LuaBotAI* ai = new LuaBotAI(pPlayer, nullptr, botRace, botClass, botGender, botLevel, pPlayer->GetMapId(), pPlayer->GetMap()->GetInstanceId(), x, y, z, pPlayer->GetOrientation(), botLogicID, botSpec);
    if (sPlayerBotMgr.AddBot(ai))
        SendSysMessage("New lua bot added.");
    else
    {
        SendSysMessage("Error spawning lua bot.");
        SetSentErrorMessage(true);
        return false;
    }
...
bool ChatHandler::HandlePartyBotMyroleCommand(char* args) {
    int myrole;
    if (!ExtractInt32(&args, myrole)) {
        SendSysMessage("Incorrect syntax. Expected integer");
        SetSentErrorMessage(true);
        return false;
    }
    else if (myrole < 0 || myrole > 2) {
        SendSysMessage("Invalid role.");
        SetSentErrorMessage(true);
        return false;
    }
    lua_State* L = sPlayerBotMgr.Lua();
    lua_pushinteger(L, myrole);
    lua_setglobal(L, "PB_LEADER_ROLE");
    SendSysMessage("Role set.");
    return true;
}

bool ChatHandler::HandlePartyBotInviteMeCommand(char* args)
{
    Player* pTarget = GetSelectedPlayer();
    if (!pTarget)
    {
    ...
    if (pTarget->AI()) {
        Group* group = pTarget->GetGroup();
        if (!group)
        {
            group = new Group;
            // new group: if can't add then delete
            if (!group->Create(pTarget->GetObjectGuid(), pTarget->GetName()))
            {
                delete group;
                SendSysMessage("Failed to create a group.");
                SetSentErrorMessage(true);
                return false;
            }
            sObjectMgr.AddGroup(group);
        }
        Player* invitee = GetSession()->GetPlayer();
        group->AddMember(GetSession()->GetPlayer()->GetObjectGuid(), GetSession()->GetPlayer()->GetName());
        group->ChangeLeader(invitee->GetObjectGuid());
        return true;
    }
    SendSysMessage("Target is not a party bot.");
    SetSentErrorMessage(true);
...
bool ChatHandler::HandlePartyBotReloadLua(char* args) {
    sPlayerBotMgr.LuaReload();
    return true;
}


bool ChatHandler::HandlePartyBotReviveAll(char* args) {
    sPlayerBotMgr.ReviveAll(GetSession()->GetPlayer());
    return true;
}
